...
// SPEC_MISMATCH: ReadableStreamDefaultTee(stream, cloneForBranch2) -> [ReadableStream, ReadableStream]
      ...
      chunkSteps(chunk) {
        queueMicrotask(() => {
          ...
          if (cloneForBranch2 && !canceled2) {
            try {
              if (!clone) throw new Error('Readable stream tee requires a clone algorithm');
              chunk2 = clone(chunk);
            } catch (error) {
