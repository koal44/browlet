...
| `HostEnqueuePromiseJob` | Places HTML-owned Promise jobs in the HTML microtask queue and prepares/cleans up their realm settings around execution | **Available on custom, adopted:** routing follows the job's queue realm; implementation continuations select their observer destination through `RuntimeContext.promises`. There is no ambient execution-owner lookup. Handlerless jobs have no script settings. Node jobs retain their native queues. See [runtime ownership](./README.md). |
