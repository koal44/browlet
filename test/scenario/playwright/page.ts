...
type ElementResult = { kind: 'elements'; elements: Element[]; error: string; };
type ValueResult = { kind: 'value'; value: string; error: string; };
type BooleanResult = { kind: 'boolean'; value: boolean; error: string; };
type CssomResult = { kind: 'cssom'; cssom: unknown; error: string; };
...
export function installBrowserHelpers(): void {
  ...
  function runQuery(query: () => QueryOutput): QueryResult {
    try {
      ...
      return { kind: 'elements', elements: [...out], error: '' };
    } catch (e) {
