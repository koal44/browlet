...
describe('interface constructor prototype fallback', () => {
  for (const prototype of [null, undefined, 0]) {
    it(`uses the newTarget realm when prototype is ${String(prototype)}`, () => {
      const { first, second, target, Example } = createBindings();
  ...
  itPassesWith('functionRealms')('reads prototype once without consulting proxy prototype traps', () => {
    const { second, target, Example } = createBindings();
    const trace: string[] = [];
  ...
  it('throws when a primitive prototype getter revokes newTarget', () => {
    const { first, target, Example } = createBindings();
    const { proxy, revoke } = Proxy.revocable(target, {
      get() { revoke(); return null; },
    });
    expect(() => { Reflect.construct(Example, [], proxy); })
  ...
  it('preserves the exception thrown by a prototype getter', () => {
    ...
    let thrown: unknown;
    try { Reflect.construct(Example, [], proxy); }
    catch (value) { thrown = value; }
    expect(thrown).toBe(error);
  });

  it('reads an object prototype only once', () => {
    const { target, Example } = createBindings();
    const prototype = {};
